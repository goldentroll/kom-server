...
class Access(k.Module):
    sync_time = 0
    timescale_map  = {'sec': 1, 'min': 60, 'hour': 3600, 'day': 86400}

    def __init__(self, 
                module : Union[k.Module, str], # the module or any python object
        ...
        k.thread(self.sync_loop_thread)
    ...
    def sync(self):
        ...
        try:
            ...
            time_since_sync = k.time() - state.get('sync_time', 0)
            if time_since_sync > self.config.sync_interval:
                self.subspace = k.module('subspace')(network=self.config.network)
                state['stakes'] = self.subspace.stakes(fmt='j', netuid=self.config.netuid)
                state['sync_time'] = k.time()
    ...
    def verify(self, input:dict) -> dict:
        ...
        if k.is_admin(address) or self.module.key.ss58_address == address:
            rate_limit = 10e42

        elif k.is_user(address):
            rate_limit = self.role2rate.get('user', 1)
        else:
            assert fn in self.module.whitelist or fn in k.helper_whitelist, f"Function {fn} not in whitelist"
        ...
        time_since_called = k.time() - user_info['last_time_called']
        ...
        user_info['last_time_called'] = k.time()
    ...
    def test(cls, key='vali::fam', base_rate=2):
        ...
        module = cls(module=k.module('module')(),  base_rate=base_rate)
        key = k.get_key(key)

        for i in range(base_rate*3):
            k.sleep(0.1)
            try:
                k.print(module.verify(input={'address': key.ss58_address, 'fn': 'info'}))
            except Exception as e:
                k.print(e)
                assert i > base_rate
