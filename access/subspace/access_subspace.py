...
class AccessSubspace(k.Module):
    ...
    def sync(self):
        sync_time  = k.time() - self.sync_time
        # if the sync time is greater than the sync interval, we need to sync
        if sync_time >  self.config.sync_interval :
            self.sync_time = k.time()
            self.subspace = k.module('subspace')(network=self.config.network, netuid=self.config.netuid)
            self.stakes = self.subspace.stakes(fmt='j')
        else:
            k.print(f"Sync time {sync_time} < {self.config.sync_interval}, skipping sync")
            return
    ...
    def verify(self, input:dict) -> dict:
        ...
        user_rate = 1 / (k.time() - user_info['last_time_called'] + 1e-10)        
    ...
    def test(cls):
        ...
        module = k.serve('module', server_name=server_name, wait_for_server=True)['name']
        client = k.connect(server_name, key='fam')
        for n in range(10):
            k.print(client.info(timeout=4))
        k.kill(server_name)
