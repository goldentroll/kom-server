...
import asyncio
import kom as k
from commune.modules.server.grpk.interceptor import ServerInterceptor
from commune.modules.server.grpk.serializer import Serializer
from commune.modules.server.grpk.proto import ServerServicer
from commune.modules.server.grpk.proto import DataBlock
import signal
from munch import Munch


class Server(ServerServicer, Serializer, k.Module):
    ...
    def __init__(
            self,
            module: Union[k.Module, object]= None,
            name = None,
            ...
            server: Optional['grpk._Server'] = None,
            verbose: bool = True,
        ...
        r""" Creates a new commune.Server object from passed arguments.
            Args:
                ...
                server (:obj:`Optional[grpk._Server]`, `required`):
        ...
        self.port = port = k.resolve_port(port)
    ...
    def set_event_loop(self, loop: 'asyncio.AbstractEventLoop' = None) -> None:
        if loop == None:
            loop = k.get_event_loop()
        self.loop = loop
    
    def set_whitelist(self, functions: List[str]):
        if functions == None:
            functions = []
        self.whitelist = list(set(functions + k.helper_functions))
    ...
    def __call__(self,
        ...
        try:
            ...
            k.print('Calling Function: '+fn, color='cyan')
            ...
            k.print(f'Exception in server: {ex}', color= 'red')
            if "There is no current event loop in thread" in str(ex):
                if verbose:
                    k.print(f'SETTING NEW ANSYNCIO LOOP', color='yellow')
            ...
            if verbose:
                k.print(f'[bold]EXCEPTION[/bold]: {ex}', color='red')
    ...
    def Forward(self, request: DataBlock, context: grpk.ServicerContext) -> DataBlock:
        r""" The function called by remote GRPC Forward requests. The Datablock is a generic formatter.
            ...
            Args:
                ...
                context (:obj:`grpk.ServicerContext`, `required`): 
                    grpc server context.
            
            Returns:
                response (k.proto.DataBlock): 
    ...
    def __del__(self):
        ...
        """
        k.print('deregister')
        k.deregister_server(name=self.name)
    ...
    def serve(self,
        ...
        def print_serve_status():
            ...
            k.print(text, color='green')
            
        
        # register the server
        if register:
            k.register_server(name=self.name, 
                                          ip=self.ip,
                                          port=self.port)
    ...
    def kill_port(port:int)-> str:
        ...
        for proc in process_iter():
            for conns in prok.connections(kind='inet'):
                if conns.laddr.port == port:
                    prok.send_signal(signal.SIGKILL) # or SIGKILL
        return port
    ...
    def port_available(cls,  port:int, ip:str = None):
        ...
        return not k.port_used(port=port, ip=ip)
