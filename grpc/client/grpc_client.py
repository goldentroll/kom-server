...
class Client(k.Module):
    ...
    def __init__( 
            ...
            loop: 'Loop' = None,
            key: 'Key' = None,
            network : 'Network' = k.default_network,
            stats = None,
        ):
    ...
    def set_client(self,
        ...
        channel = grpk.aio.insecure_channel(
            self.endpoint,
            options=[('grpk.max_send_message_length', -1),
                     ('grpk.max_receive_message_length', -1),
                     ('grpk.keepalive_time_ms', 100000)])
    ...
    async def async_forward(
        ...
        **added_kwargs
        ...
        if verbose:
            k.print(f"SENDING --> {self.endpoint}::fn::({fn}), timeout: {timeout} data: {data}",color=random_color)
    ...
    def sync_wrapper(self,fn:'asyncio.callable', loop = None) -> 'callable':
        '''
        Convert Async funciton to Synk.
    ...
    def virtual(self):
        return k.virtual_client(module = self)
