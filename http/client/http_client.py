...
class Client(k.Module):
    ...
    def __init__( 
        ...
        self.loop = k.get_event_loop() if loop == None else loop
        self.set_client(ip =ip,port = port)
        self.serializer = k.serializer()
        self.key = k.get_key(key)
        self.my_ip = k.ip()
        self.network = k.resolve_network(network)
        self.start_timestamp = k.timestamp()

    
    def age(self):
        return  self.start_timestamp - k.timestamp()
    ...
    def set_client(self,
        ...
        self.ip = ip if ip else k.default_ip
        self.port = port if port else k.free_port() 
        if verbose:
            k.print(f"Connecting to {self.ip}:{self.port}", color='green')
